...
def create_parser():
    ...
    parser.add_argument("--d_ff", type=int, default=700)
    ...
    parser.add_argument("--d_qk", type=int, default=30)
    ...
    parser.add_argument("--embed_method", type=str, default="spatio-temporal", help="Embedding method")
    parser.add_argument("--local_self_attn", type=str, default="full", help="Local self-attention type")
    parser.add_argument("--local_cross_attn", type=str, default="full", help="Local cross-attention type")
    parser.add_argument("--global_self_attn", type=str, default="full", help="Global self-attention type")
    parser.add_argument("--global_cross_attn", type=str, default="full", help="Global cross-attention type")
    parser.add_argument("--start_token_len", type=int, default=0, help="Start token length")
    parser.add_argument("--attn_factor", type=float, default=1.0, help="Factor for attention scaling")
    parser.add_argument("--dropout_attn_out", type=float, default=0.1, help="Dropout rate for attention output")
    parser.add_argument("--dropout_attn_matrix", type=float, default=0.1, help="Dropout rate for attention matrix")
    parser.add_argument("--dropout_qkv", type=float, default=0.1, help="Dropout rate for QKV (query, key, value) attention layers")
    parser.add_argument("--pos_emb_type", type=str, default="learned", help="Tipo de embedding posicional (por ejemplo, 'learned' o 'fixed')")
    parser.add_argument("--no_final_norm", action="store_true", help="Desactivar normalización final")
    parser.add_argument("--performer_kernel", type=str, choices=["linear", "gaussian"], default="linear", help="Tipo de kernel para Performer (opciones: 'linear', 'gaussian')")
    parser.add_argument("--performer_redraw_interval", type=int, default=100, help="Intervalo para el redibujado de Performer (valor entero)")
    parser.add_argument("--attn_time_windows", type=int, default=5, help="Número de ventanas de tiempo para la atención (valor entero)")
    parser.add_argument("--use_shifted_time_windows", action='store_true', help="Usar ventanas de tiempo desplazadas")
    parser.add_argument("--norm", default="layer", choices=["layer", "batch", "none"], help="Tipo de normalización (layer, batch, none)")
    parser.add_argument("--activation", default="relu", choices=["relu", "gelu", "tanh", "sigmoid"], help="Función de activación a usar")
    parser.add_argument("--init_lr", type=float, default=1e-3, help="Tasa de aprendizaje inicial")
    parser.add_argument("--initial_downsample_convs", type=int, default=1, help="Número de convoluciones de downsampling iniciales")
    parser.add_argument("--intermediate_downsample_convs", type=int, default=1, help="Número de convoluciones de downsampling intermedias")
    parser.add_argument("--class_loss_imp", type=float, default=1.0, help="Importancia de la pérdida de clase")
    parser.add_argument("--recon_loss_imp", type=float, default=1.0, help="Importancia de la pérdida de reconstrucción")
    parser.add_argument("--time_emb_dim", type=int, default=32, help="Dimensión de la representación temporal")
    parser.add_argument("--no_val", action='store_true', help="Desactivar la validación")
    parser.add_argument("--no_time", action='store_true', help="Desactivar el uso del tiempo")
    parser.add_argument("--no_space", action='store_true', help="Desactivar el uso del espacio")
    parser.add_argument("--no_given", action='store_true', help="Desactivar el uso de la entrada dada")
    parser.add_argument("--recon_mask_skip_all", action='store_true', help="Habilitar el enmascarado de reconstrucción en todo")
    parser.add_argument("--recon_mask_max_seq_len", type=int, default=128, help="Longitud máxima de secuencia para el enmascarado de reconstrucción")
    parser.add_argument("--recon_mask_drop_seq", type=int, default=10, help="Secuencias de caída para el enmascarado de reconstrucción")
    parser.add_argument("--recon_mask_drop_standard", type=int, default=10, help="Standard drop rate for mask reconstruction")
    parser.add_argument("--recon_mask_drop_full", type=int, default=10, help="Full drop rate for mask reconstruction")
    parser.add_argument("--position_emb", type=str, choices=["t2v", "abs"], default="t2v", help="Position embedding method")
